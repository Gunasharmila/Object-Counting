...
def load_detection_model(model_path):
    ...
    try:
        # TensorFlow 2.x method to load the model
        model = tf.saved_model.load(model_path).signatures['serving_default']
        return model
    except AttributeError:
        st.error("TensorFlow 'saved_model' attribute not found. Please ensure you're using TensorFlow 2.x.")
        return None
...
if __name__ == "__main__":
    ...
    if detection_model:
        st.subheader("Starting object detection. Press 'q' to quit.")
        ...
        st.error("Model could not be loaded. Please check the TensorFlow installation and model path.")
